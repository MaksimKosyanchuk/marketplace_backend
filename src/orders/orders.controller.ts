...
export class OrdersController {
    ...
    @Post('checkout')
    @ApiOperation({ summary: 'Оформить заказ из текущей корзины пользователя' })
    @ApiResponse({
        status: 201,
    ...
    checkout(@Req() req: Request & { user: { id: string } }) {
        return this.ordersService.checkout(req.user.id);
    }

    @Post(':id/pay')
    @ApiOperation({ summary: 'Оплатить заказ по ID' })
    @ApiParam({ name: 'id', description: 'ID заказа', example: 'ord_789ghi' })
    @ApiResponse({
        status: 200,
        description: 'Статус заказа обновлен (Оплачен)',
        type: OrderResponseDto,
    })
    @ApiResponse({ status: 400, description: 'Нельзя оплатить заказ в текущем статусе' })
    @ApiResponse({ status: 401, description: 'Неавторизован' })
    ...
    @Post(':id/cancel')
    @ApiOperation({ summary: 'Отменить заказ по ID' })
    @ApiParam({ name: 'id', description: 'ID заказа', example: 'ord_789ghi' })
    @ApiResponse({
        status: 200,
    ...
    @ApiResponse({ status: 400, description: 'Нельзя отменить выполняемый или уже завершенный заказ' })
    @ApiResponse({ status: 401, description: 'Неавторизован' })
    @ApiResponse({ status: 404, description: 'Заказ не найден' })
    cancelOrder(
        @Req() req: Request & { user: { id: string } },
    ...
    @ApiResponse({ status: 401, description: 'Неавторизован' })
    findMyOrders(@Req() req: Request & { user: { id: string } }) {
    ...
    @ApiResponse({ status: 401, description: 'Неавторизован' })
    @ApiResponse({ status: 403, description: 'Доступ к чужому заказу запрещен' })
    @ApiResponse({ status: 404, description: 'Заказ не найден' })
    findOne(
    ...
    @Roles(Role.ADMIN)
    @Get()
    @ApiOperation({ summary: 'Получить список всех заказов в системе (Только ADMIN)' })
    @ApiResponse({
        status: 200,
    ...
    @ApiResponse({ status: 400, description: 'Невалидный статус' })
    @ApiResponse({ status: 401, description: 'Неавторизован' })
